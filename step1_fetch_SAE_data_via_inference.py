...
def search_latent_features_by_model(query, model, source_set): 
    ...
    if not os.path.exists('json6/' + model + '/' + source_set + '/'+ query):
        os.makedirs('json6/' + model + '/'+ source_set + '/' + query)  
    
    # Save the JSON response to a file in the 'json' folder
    filename = 'json6/' + model + '/'+ source_set + '/' + query + '/explanation_for_query_' + query + '.json'
...
def search_explanations_by_feature(feature,query):
    ...
    filename = 'json6/'+model+'/'+ source_set + '/'+query+'/data_for_feature_' + str(index) + '.json'
    with open(filename, 'w') as json_file:
